...
st.markdown("Use o campo abaixo para pesquisar um ativo da B3 (ex: PETR4, ITSA4, BBAS3)")
    ...
    if stock_info and not hist_data.empty:
        ...
        with tab1:
            ...
            if valid_models:
                ...
                with col_val2:
                    st.markdown(f"##### Preço Justo Médio: <font color='blue'>R$ {avg_fair_price:.2f}</font>", unsafe_allow_html=True)
            ...
            # ✅ CORREÇÃO FINAL E DEFINITIVA PARA O GRÁFICO DE DIVIDENDOS ✅
            dividends_data = hist_data[hist_data['Dividends'] > 0]
            if not dividends_data.empty:
                # 1. Filtra apenas dividendos com data de pagamento ATÉ HOJE.
                today = pd.to_datetime(datetime.now().date())
                dividends_past = dividends_data[dividends_data.index <= today]
                
                if not dividends_past.empty:
                    # 2. Agrupa por ano de forma segura.
                    annual_dividends = dividends_past.groupby(dividends_past.index.year)['Dividends'].sum()
                    
                    # 3. Pega o último preço de fechamento para cada ano correspondente.
        ...
        with tab2:
            ...
            fig_price.update_layout(title=f'Gráfico de Preços para {ticker_input} (Últimos 2 Anos)', yaxis_title='Preço (R$)', xaxis_rangeslider_visible=False, height=400, legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
