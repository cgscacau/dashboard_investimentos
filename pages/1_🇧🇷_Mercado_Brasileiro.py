...
@st.cache_data(ttl=1800) # Função "mãe" com cache
...
if ticker_input:
    ...
    if stock_info and not hist_data.empty:
        current_price = stock_info.get('currentPrice') or hist_data['Close'].iloc[-1]; sector_pt = SECTOR_TRANSLATIONS.get(stock_info.get('industry', stock_info.get('sector', 'N/A')), stock_info.get('industry', stock_info.get('sector', 'N/A'))); hist_2y = hist_data.loc[hist_data.index > (datetime.now() - timedelta(days=730))].copy(); summary_translated = translate_text_safely(stock_info.get('longBusinessSummary')); change_pct_manual = None; previous_close = stock_info.get('previousClose')
        ...
        cols_header[0].metric("Preço Atual", f"R$ {current_price:.2f}", delta=delta_text); cols_header[1].metric("Setor", sector_pt); cols_header[2].metric("Mín. 52 Semanas", f"R$ {stock_info.get('fiftyTwoWeekLow', 0):.2f}"); cols_header[3].metric("Máx. 52 Semanas", f"R$ {stock_info.get('fiftyTwoWeekHigh', 0):.2f}")
