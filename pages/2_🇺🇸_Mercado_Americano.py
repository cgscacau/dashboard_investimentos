...
def get_stock_data(ticker):
    return get_stock_data_raw(ticker)
...
def translate_text_safely(text):
    ...
    try: translator = Translator(); return translator.translate(text, dest='pt').text
...
def get_valuation_models(info, current_price):
    results = {}; eps = info.get('trailingEps'); bvps = info.get('bookValue')
...
def create_valuation_gauge(avg_fair_price, current_price):
    ...
    fig = go.Figure(go.Indicator(mode="gauge+number+delta", value=current_price, title={'text': "Preço Atual vs. Preço Justo"}, delta={'reference': avg_fair_price, 'increasing': {'color': "red"}, 'decreasing': {'color': "green"}}, gauge={'axis': {'range': [None, avg_fair_price * 1.5]}, 'steps': [{'range': [0, avg_fair_price * 0.8], 'color': "lightgreen"}, {'range': [avg_fair_price * 0.8, avg_fair_price * 1.2], 'color': "lightyellow"}, {'range': [avg_fair_price * 1.2, avg_fair_price * 1.5], 'color': "lightcoral"}], 'threshold': {'line': {'color': "black", 'width': 4}, 'thickness': 0.75, 'value': avg_fair_price}}))
    fig.update_layout(height=300, margin=dict(t=50, b=50)); return fig
...
if ticker_input:
    ...
    if stock_info and not hist_data.empty:
        current_price = stock_info.get('currentPrice') or hist_data['Close'].iloc[-1]; category_en = stock_info.get('industry', stock_info.get('sector', 'N/A')); sector_pt = SECTOR_TRANSLATIONS.get(category_en, category_en); hist_2y = hist_data.loc[hist_data.index > (datetime.now() - timedelta(days=730))].copy(); summary_translated = translate_text_safely(stock_info.get('longBusinessSummary')); change_pct_manual = None; previous_close = stock_info.get('previousClose')
        if previous_close and current_price and previous_close > 0: change_pct_manual = ((current_price / previous_close) - 1)
        delta_text = f"{change_pct_manual * 100:.2f}%" if change_pct_manual is not None else ""; dy_value = f"{(stock_info.get('trailingAnnualDividendYield', 0) * 100):.2f}%" if stock_info.get('trailingAnnualDividendYield') else "N/A"
        st.header(f"{stock_info.get('longName', 'N/A')} ({stock_info.get('symbol', 'N/A')})", divider='rainbow'); cols_header = st.columns(4)
        cols_header[0].metric("Preço Atual", f"$ {current_price:.2f}", delta=delta_text); cols_header[1].metric("Setor", sector_pt); cols_header[2].metric("Mín. 52 Semanas", f"$ {stock_info.get('fiftyTwoWeekLow', 0):.2f}"); cols_header[3].metric("Máx. 52 Semanas", f"$ {stock_info.get('fiftyTwoWeekHigh', 0):.2f}")
        tab1, tab2, tab3 = st.tabs(["📜 Análise Fundamentalista", "📊 Análise Técnica Avançada", "📄 Dados Completos"])
        with tab1:
            st.subheader("Principais Indicadores"); fund_data = {"P/L": f"{stock_info.get('trailingPE'):.2f}" if stock_info.get('trailingPE') else "N/A", "P/VP": f"{stock_info.get('priceToBook'):.2f}" if stock_info.get('priceToBook') else "N/A", "Dividend Yield (12M)": dy_value, "ROE (%)": f"{stock_info.get('returnOnEquity', 0) * 100:.2f}%" if stock_info.get('returnOnEquity') else "N/A", "Valor de Mercado": format_number(stock_info.get('marketCap')), "Dívida/Patrimônio": f"{stock_info.get('debtToEquity'):.2f}" if stock_info.get('debtToEquity') else "N/A"}
            cols_fund = st.columns(3); _ = [cols_fund[i % 3].metric(key, value) for i, (key, value) in enumerate(fund_data.items())]
            st.markdown("---"); st.subheader("Análise de Valuation"); valuation_results = get_valuation_models(stock_info, current_price); valid_models = {k: v for k, v in valuation_results.items() if v is not None and v > 0}
            if valid_models:
                ...
                with col_val2: st.markdown(f"##### Preço Justo Médio: <font color='blue'>$ {avg_fair_price:.2f}</font>", unsafe_allow_html=True); st.markdown(f"##### Potencial de Valorização: <font color='{'green' if upside > 0 else 'red'}'>{upside:.2f}%</font>", unsafe_allow_html=True); st.markdown("---"); st.write("**Modelos Utilizados:**");_ = [st.write(f"- {model}: **$ {price:.2f}**") for model, price in valid_models.items()]
            else: st.warning("Não foi possível calcular um preço justo para este ativo com os dados disponíveis.")
            st.markdown("---"); st.subheader("Histórico de Dividendos (Últimos Anos)"); dividends_past = hist_data[(hist_data['Dividends'] > 0) & (hist_data.index <= datetime.now())]
            if not dividends_past.empty:
                ...
                for year in annual_dividends.index:
                    dividend_value = annual_dividends.get(year, 0); close_price = annual_close_price.get(year); text = f"$ {dividend_value:.2f}"
