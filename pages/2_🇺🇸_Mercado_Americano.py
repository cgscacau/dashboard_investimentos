# pages/2_🇺🇸_Mercado_Americano.py
...
# --- FUNÇÕES DE APOIO ---
# Dicionário para traduções pontuais e estáveis
SECTOR_TRANSLATIONS = {
...
# ✅ MUDANÇA: Função de busca sem o sufixo '.SA'
@st.cache_data(ttl=3600)
def get_stock_data(ticker):
    stock = yf.Ticker(ticker) # Apenas o ticker, sem sufixo
    try:
        info = stock.info
...
# ✅ MUDANÇA: Símbolo da moeda alterado para Dólar
...
# --- FUNÇÕES DE ANÁLISE FUNDAMENTALISTA (idênticas) ---
...
if ticker_input:
    ...
    if stock_info and not hist_data.empty:
        ...
        cols_header[0].metric("Preço Atual", f"$ {current_price:.2f}", delta=delta_text)
        cols_header[1].metric("Setor", sector_pt)
        cols_header[2].metric("Mín. 52 Semanas", f"$ {stock_info.get('fiftyTwoWeekLow', 0):.2f}")
        cols_header[3].metric("Máx. 52 Semanas", f"$ {stock_info.get('fiftyTwoWeekHigh', 0):.2f}")
        ...
        with tab1:
            ...
            fund_data = {
                "P/L": f"{stock_info.get('trailingPE'):.2f}" if stock_info.get('trailingPE') else "N/A",
                "P/VP": f"{stock_info.get('priceToBook'):.2f}" if stock_info.get('priceToBook') else "N/A",
                "Dividend Yield (12M)": dy_value,
                "ROE (%)": f"{stock_info.get('returnOnEquity', 0) * 100:.2f}%" if stock_info.get('returnOnEquity') else "N/A",
                "Valor de Mercado": format_number(stock_info.get('marketCap')),
                "Dívida/Patrimônio": f"{stock_info.get('debtToEquity'):.2f}" if stock_info.get('debtToEquity') else "N/A"
            }
            cols_fund = st.columns(3)
            for i, (key, value) in enumerate(fund_data.items()):
                cols_fund[i % 3].metric(key, value)

            st.markdown("---")
            st.subheader("Análise de Valuation")
            valuation_results = get_valuation_models(stock_info, current_price)
            valid_models = {k: v for k, v in valuation_results.items() if v is not None and v > 0}
            if valid_models:
                avg_fair_price = sum(valid_models.values()) / len(valid_models)
                upside = ((avg_fair_price / current_price) - 1) * 100
                col_val1, col_val2 = st.columns([1, 2])
                with col_val1:
                    gauge_fig = create_valuation_gauge(avg_fair_price, current_price)
                    st.plotly_chart(gauge_fig, use_container_width=True)
                with col_val2:
                    st.markdown(f"##### Preço Justo Médio: <font color='blue'>$ {avg_fair_price:.2f}</font>", unsafe_allow_html=True)
                    st.markdown(f"##### Potencial de Valorização: <font color='{'green' if upside > 0 else 'red'}'>{upside:.2f}%</font>", unsafe_allow_html=True)
                    st.markdown("---"); st.write("**Modelos Utilizados:**")
                    for model, price in valid_models.items():
                        st.write(f"- {model}: **$ {price:.2f}**")
            else:
                st.warning("Não foi possível calcular um preço justo para este ativo com os dados disponíveis.")

            st.markdown("---")
            st.subheader("Histórico de Dividendos (Últimos Anos)")
            dividends_past = hist_data[(hist_data['Dividends'] > 0) & (hist_data.index <= datetime.now())]
            if not dividends_past.empty:
                annual_dividends = dividends_past.groupby(dividends_past.index.year)['Dividends'].sum()
                annual_close_price = hist_data.groupby(hist_data.index.year)['Close'].last()
                texts = []
                for year in annual_dividends.index:
                    dividend_value = annual_dividends.get(year, 0)
                    close_price = annual_close_price.get(year)
                    text = f"$ {dividend_value:.2f}"
                    if close_price and close_price > 0 and dividend_value > 0:
                        yield_value = (dividend_value / close_price) * 100
                        text += f"<br>{yield_value:.2f}%"
                    texts.append(text)
                fig_div = go.Figure()
                fig_div.add_trace(go.Bar(x=annual_dividends.index, y=annual_dividends.values, text=texts, textposition='auto', name='Dividendos'))
                fig_div.update_xaxes(type='category')
                fig_div.update_layout(title_text='Dividendos Pagos por Ano (Valor e % Yield)', height=350, uniformtext_minsize=8, uniformtext_mode='hide')
                st.plotly_chart(fig_div, use_container_width=True)
            else:
                st.info("Este ativo não registrou pagamento de dividendos nos últimos 5 anos.")

            st.subheader("Descrição da Empresa")
            if summary_translated and summary_translated != "Descrição não disponível.":
                st.markdown(f"<p style='font-size: 20px;'>{summary_translated}</p>", unsafe_allow_html=True)
            else:
                st.info("A descrição da empresa não foi fornecida pela fonte de dados.")
        ...
        with tab2:
            ...
            fig_price.update_layout(title=f'Gráfico de Preços para {ticker_input} (Últimos 2 Anos)', yaxis_title='Preço ($)', xaxis_rangeslider_visible=False, height=400, legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
        ...
        with tab3:
            st.subheader("Dados Fundamentais Completos (JSON)")
            st.info("Estes são todos os dados brutos sobre o ativo retornados pela API yfinance.")
            st.json(stock_info)

            st.subheader("Histórico de Preços e Dividendos (Tabela)")
